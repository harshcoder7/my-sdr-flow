...
def show_csv_converter():
    ...
    if uploaded_file is not None:
        try:
            ...
            if 'csv_file_key' not in st.session_state or st.session_state.csv_file_key != file_key:
            ...
            available_company_cols = [col for col in company_columns if col in df.columns]
            available_person_cols = [col for col in person_columns if col in df.columns]
            
            st.info(f"📊 Available company columns: {len(available_company_cols)}/{len(company_columns)}")
            st.info(f"👥 Available person columns: {len(available_person_cols)}/{len(person_columns)}")
            
            # Check for required columns
            missing_required = [col for col in CSV_CONFIG["required_columns"] if col not in df.columns]
            if missing_required:
                ...
                if st.button("🔄 Convert to JSON", type="primary"):
                    result = process_csv_to_json(df, available_company_cols, available_person_cols)
    ...
    with col1:
        ...
        if st.button("💾 Save Data for Workflow", type="primary", key="save_workflow"):
            if save_workflow_data(result, "CSV Converter"):
                st.success("✅ Data saved to workflow! You can now access it from other sections.")
            else:
