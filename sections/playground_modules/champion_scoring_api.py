            ...
            for item in inner_outputs:
                ...
                for msg in messages:
                    ...
                    if "```json" in message_text:
                        json_block = (
                            message_text.split("```json")[1].split("```")[0].strip()
                        )
...
def show_champion_scoring_playground():
    ...
    if input_method == "Manual Input":
        st.info("🚧 Coming soon...")
        st.markdown(
            "Manual input interface is currently under development. Please use JSON input for now."
        )
    ...
    if st.button(
        "🎯 Calculate Champion Score", key="champion_api_call", type="primary"
    ):
        ...
        try:
            ...
            if input_method == "JSON Input":
                ...
                missing_fields = [
                    field for field in required_fields if not parsed_json.get(field)
                ]
                if missing_fields:
        ...
        with st.spinner("Calculating champion score..."):
            response = api_client.make_request(
                ...
                process_fn=extract_champion_score_result,
            )
